...
int sc_new_stream ( struct sc_stream_t *stream, struct sc_context_t *context, int encrypt )
{
    ...
    if(!(stream->aes = EVP_CIPHER_CTX_new()))
            handleErrors();
    ...
    if ( encrypt )
    {
        if(1 != EVP_EncryptInit_ex(stream->aes, EVP_aes_256_cbc(), NULL, context->aeskey, stream->iv)) {
            EVP_CIPHER_CTX_free ( stream->aes );
            memset ( rawkey, '\0', sizeof ( rawkey ) );
            handleErrors();
        }
    } else
    {
        memcpy ( stream->aeskey, context->aeskey, AES256_KEYLEN);
        memset ( rawkey, '\0', sizeof ( rawkey ) );
    }
...
static int sc_encrypt_data ( struct sc_stream_t *stream, const uint8_t * src, int len )
{
    ...
    if(1 != EVP_EncryptUpdate(stream->aes, stream->processed + opos, &len2, workbuf, AES256_BLOCKLEN))
        handleErrors();
...
static int sc_decrypt_data ( struct sc_stream_t *stream, const uint8_t * src, int len )
{
    ...
    int opos = 0;
    int len2 = 0;
    uint8_t workbuf[AES256_BLOCKLEN * 2];
    ...
    if ( ~stream->flags & SC_STREAM_RECV_RXNONCE )
    {
        ...
        if(1 != EVP_DecryptInit_ex(stream->aes, EVP_aes_256_cbc(), NULL, stream->aeskey, stream->iv)) {
            EVP_CIPHER_CTX_free ( stream->aes );
            handleErrors();
        }
    ...
    {
        if(1 != EVP_DecryptUpdate(stream->aes, workbuf, &len2, stream->unconsumed, AES256_BLOCKLEN))
            handleErrors();
        if ( stream->expected_len )
        {
            vlen = stream->expected_len > AES256_BLOCKLEN ? AES256_BLOCKLEN : stream->expected_len;
            memcpy ( stream->processed + opos, workbuf + len2, vlen );
            opos += vlen;
            stream->expected_len -= vlen;

        } else
        {
            stream->expected_len = ( workbuf[len2 + 0] << 8 ) | workbuf[len2 + 1];
            vlen =
                stream->expected_len >
                AES256_BLOCKLEN - 2 ? AES256_BLOCKLEN - 2 : stream->expected_len;
            memcpy ( stream->processed + opos, workbuf  + len2 + 2, vlen );
            opos += vlen;
            stream->expected_len -= vlen;
        }
    }
    ...
    {
        if(1 != EVP_DecryptUpdate(stream->aes, workbuf, &len2, src + ipos, AES256_BLOCKLEN))
            handleErrors();
